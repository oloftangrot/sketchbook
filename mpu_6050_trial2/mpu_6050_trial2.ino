...
class float_sat {
  public:
      float_sat(float_sat& f) { // Copy constructor
        min=f.min;
        max=f.max;
        val=f.val; };
    float_sat(float min, float max ) {
      this->min = min;
      this->max = max;
      this->val = 0; };
    float_sat operator=( const float& f) { 
      if ( f>max ) val=max; 
      else if (f<min) val=min; 
      else val=f;}
    float_sat operator=( const float_sat& f) { 
      if ( f.val>max ) val=max; 
      else if (f.val<min) val=min; 
      else val=f.val;}
      inline operator float() { return val; }
  private:
    float_sat(); // Hide default contructor.
    float min, max, val;
};


Servo mirror;
const int middle = 90; // Define the mirror servo rest position
float_sat phi_sat( -M_PI_4, M_PI_4 ); // Set the limits for the saturated correction angle.
...
{      
  ...
  mirror.attach(10) ; // Use pin 10
  mirror.write ( middle ); // Set the middle position
...
{
  int error;
  const float g_conv = 16384.f/9.81f;  // FDS -> [N/s^2]
  const float L_conv = 16384.f/250.f;
  float phi;
  double dT;
  ...
  Serial.print( accel_t_gyro.value.x_accel / g_conv, 2 );
  Serial.print( F(", ") );
  Serial.print( accel_t_gyro.value.y_accel / g_conv, 2 );
  Serial.print( F(", ") );
  Serial.print( accel_t_gyro.value.z_accel / g_conv, 2 );
  Serial.print( F(" [N/s^2]") );
  Serial.println( F("") );

  phi_sat = static_cast<float> ( atan2( accel_t_gyro.value.x_accel, accel_t_gyro.value.y_accel ) );
  mirror.write( middle + 180.f * phi_sat / M_PI);
  Serial.print( F("yaw (sturated): " ) );
  Serial.print( phi_sat * 180.f/ M_PI, 1 );
  Serial.print( F(" [deg]") );
  Serial.println( F("") );
  ...
  Serial.print( accel_t_gyro.value.x_gyro / L_conv, 2 );
  Serial.print( F(", ") );
  Serial.print( accel_t_gyro.value.y_gyro / L_conv, 2 );
  Serial.print( F(", ") );
  Serial.print( accel_t_gyro.value.z_gyro / L_conv, 2 );
